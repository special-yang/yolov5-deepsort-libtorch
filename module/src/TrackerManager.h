...
public:
    ...
    update(const std::vector<cv::Rect2f> &dets,
        ...
        // split track set into confirmed and
        // unconfirmed tracks
        std::vector<int> unmatched_trks;
    ...
    std::vector<Track> visible_tracks() {
        ...
        for (auto &t : data) {
            ...
            //     ret.push_back(res);
            // }
            if (t.kalman.state() == TrackState::Confirmed && t.kalman.get_time_since_update()<4) {
                std::cout<<"id "<<t.kalman.id()<<" time_since_update "<<t.kalman.get_time_since_update()<<std::endl;
                Track res{t.kalman.id(), bbox};
                ret.push_back(res);
            }
        }
